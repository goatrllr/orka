...
orca:register_property(Key, Pid, #{property => Name, value => Value}).

%% Find all entries with a specific property value
orca:find_by_property(PropertyName, PropertyValue).

%% Find entries with a property value filtered by type
orca:find_by_property(Type, PropertyName, PropertyValue).
...
orca:register({global, service, translator_1}, TranslatorPid1, 
...
orca:register({global, service, translator_2}, TranslatorPid2, 
    #{tags => [service, online, translator]}),
orca:register_property({global, service, translator_2}, TranslatorPid2, 
    #{property => capacity, value => 150}).

orca:register({global, service, translator_3}, TranslatorPid3, 
    #{tags => [service, online, translator]}),
orca:register_property({global, service, translator_3}, TranslatorPid3, 
...
orca:find_by_property(service, capacity, 150).
...
select_translator(RequiredCapacity) ->
    ...
    case orca:find_by_property(service, capacity, RequiredCapacity) of
        [_|_] = Instances ->
...
orca:register({global, resource, db_west_1}, DbPidWest1,
    #{tags => [database, replica]}),
orca:register_property({global, resource, db_west_1}, DbPidWest1,
    #{property => region, value => "us-west"}).

orca:register({global, resource, db_west_2}, DbPidWest2,
    #{tags => [database, replica]}),
orca:register_property({global, resource, db_west_2}, DbPidWest2,
    #{property => region, value => "us-west"}).

orca:register({global, resource, db_east}, DbPidEast,
    #{tags => [database, replica]}),
orca:register_property({global, resource, db_east}, DbPidEast,
    #{property => region, value => "us-east"}).

orca:register({global, resource, db_europe}, DbPidEurope,
    #{tags => [database, replica]}),
orca:register_property({global, resource, db_europe}, DbPidEurope,
...
orca:find_by_property(resource, region, "us-west").
%% Result: [
...
orca:count_by_property(region, "us-east").
%% Result: 1
```
...
get_local_db_connection(UserRegion) ->
    case orca:find_by_property(resource, region, UserRegion) of
        [_|_] = LocalReplicas ->
        ...
        [] ->
            ...
            case orca:find_by_property(resource, region, '_') of
                [] -> {error, no_replicas};
                [_|_] = AllReplicas ->
...
orca:register({global, worker, w1}, WorkerPid1,
    #{tags => [worker, online]}),
orca:register_property({global, worker, w1}, WorkerPid1,
    #{property => health, value => ok}).

orca:register({global, worker, w2}, WorkerPid2,
    #{tags => [worker, online]}),
orca:register_property({global, worker, w2}, WorkerPid2,
    #{property => health, value => ok}).

orca:register({global, worker, w3}, WorkerPid3,
    #{tags => [worker, online]}),
orca:register_property({global, worker, w3}, WorkerPid3,
    #{property => health, value => warning}).

%% Later: update health status
%% When w1 becomes unhealthy, remove and re-register
orca:unregister({global, worker, w1}),
orca:register({global, worker, w1}, WorkerPid1,
...
orca:count_by_property(health, ok).
%% Result: 1

%% Find all degraded workers
orca:find_by_property(worker, health, degraded).
...
orca:register({global, service, api_v1}, ApiPidV1,
    #{tags => [service, api, online]}),
orca:register_property({global, service, api_v1}, ApiPidV1,
    #{property => api_version, value => "1.0.0"}).

%% v2 API adds streaming support
orca:register({global, service, api_v2}, ApiPidV2,
    #{tags => [service, api, online]}),
orca:register_property({global, service, api_v2}, ApiPidV2,
    #{property => api_version, value => "2.0.0"}).
orca:register_property({global, service, api_v2}, ApiPidV2,
    #{property => feature, value => streaming}).

%% v3 API adds advanced features
orca:register({global, service, api_v3}, ApiPidV3,
    #{tags => [service, api, online]}),
orca:register_property({global, service, api_v3}, ApiPidV3,
...
orca:find_by_property(service, feature, streaming).
%% Result: [
...
orca:find_by_property(service, feature, webhooks).
...
get_capability(Feature) ->
    case orca:find_by_property(service, feature, Feature) of
        [_|_] = Instances ->
            {ok, Instances};
        [] ->
...
route_request(RequiredFeatures) ->
    AvailableInstances = orca:find_by_property(service, api_version, '_'),
...
orca:register({user, "alice@example.com", session_1}, AliceWeb,
    #{tags => [session, user, alice, online]}),
orca:register_property({user, "alice@example.com", session_1}, AliceWeb,
    #{property => device_type, value => web}).

orca:register({user, "alice@example.com", session_2}, AliceMobile,
    #{tags => [session, user, alice, online]}),
orca:register_property({user, "alice@example.com", session_2}, AliceMobile,
    #{property => device_type, value => mobile}).

orca:register({user, "alice@example.com", session_3}, AliceDesktop,
    #{tags => [session, user, alice, online]}),
orca:register_property({user, "alice@example.com", session_3}, AliceDesktop,
    #{property => device_type, value => desktop}).

%% User "bob@example.com" has one session
BobMobile = spawn_session_handler(bob, mobile),
orca:register({user, "bob@example.com", session_1}, BobMobile,
    #{tags => [session, user, bob, online]}),
orca:register_property({user, "bob@example.com", session_1}, BobMobile,
    #{property => device_type, value => mobile}).
```
...
send_notification_to_devices(UserId, Devices, Message) ->
    lists:foreach(fun(DeviceType) ->
        case orca:find_by_property(device_type, DeviceType) of
            [_|_] = Sessions ->
...
orca:register({global, pool, db_pool_1}, Pool1Pid,
    #{tags => [resource, connection_pool, database]}),
orca:register_property({global, pool, db_pool_1}, Pool1Pid,
    #{property => max_connections, value => 100}).
orca:register_property({global, pool, db_pool_1}, Pool1Pid,
    #{property => active_connections, value => 45}).

orca:register({global, pool, db_pool_2}, Pool2Pid,
    #{tags => [resource, connection_pool, database]}),
orca:register_property({global, pool, db_pool_2}, Pool2Pid,
    #{property => max_connections, value => 150}).
orca:register_property({global, pool, db_pool_2}, Pool2Pid,
    #{property => active_connections, value => 148}).

orca:register({global, pool, db_pool_3}, Pool3Pid,
    #{tags => [resource, connection_pool, database]}),
orca:register_property({global, pool, db_pool_3}, Pool3Pid,
...
orca:find_by_property(pool, active_connections, 45).
...
orca:find_by_property(pool, max_connections, 150).
%% Result: [{{global, pool, db_pool_2}, <0.311.0>, {...}}]

%% Count pools
orca:count_by_property(max_connections, 100).
%% Result: 1
```
