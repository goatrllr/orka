-module(orca_gpt_regression_SUITE).
...
test_property_stats_filters_by_type(Config) ->
	...
	{ok, _} = orca:register(Svc1, #{tags => [service]}),
	ok = orca:register_property(Svc1, self(), #{property => status, value => healthy}),

	{ok, _} = orca:register(Svc2, #{tags => [service]}),
	ok = orca:register_property(Svc2, self(), #{property => status, value => healthy}),

	{ok, _} = orca:register(Res1, #{tags => [resource]}),
	ok = orca:register_property(Res1, self(), #{property => status, value => healthy}),

	%% Expect only services to be counted; actual code returns 3 because it ignores the filter.
	StatsForServices = orca:property_stats(service, status),
...
test_register_property_overwrites_previous_value(Config) ->
	...
	{ok, _} = orca:register(Key, #{tags => [service]}),
	ok = orca:register_property(Key, self(), #{property => version, value => <<"1.0.0">>}),
	ok = orca:register_property(Key, self(), #{property => version, value => <<"2.0.0">>}),

	%% Expect only the latest value to be visible; current code still returns the old one.
	[] = orca:find_by_property(version, <<"1.0.0">>),
	[{Key, _Pid, _}] = orca:find_by_property(version, <<"2.0.0">>),
	Config.
